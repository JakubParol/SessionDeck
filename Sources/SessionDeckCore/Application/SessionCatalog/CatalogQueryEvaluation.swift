...
public enum CatalogQueryEvaluation {
    public static func query(
        sessions: [SessionSummary],
        request: CatalogQueryRequest = CatalogQueryRequest()
    ) -> [SessionSummary] {
        SessionCatalogOrdering.sort(
    ...
    public static func filterOptions(for sessions: [SessionSummary]) -> CatalogFilterOptions {
    ...
    private static func projectOptions(for sessions: [SessionSummary]) -> [CatalogProjectFilterOption] {
    ...
    private static func projectFilter(for group: ProjectNavigationGroup) -> CatalogProjectFilter {
        switch group.kind {
        case .project:
    ...
    private static func sourceOptions(for sessions: [SessionSummary]) -> [CatalogSourceFilterOption] {
    ...
    private static func profileOptions(for sessions: [SessionSummary]) -> [CatalogProfileFilterOption] {
    ...
    private static func parseStatusOptions(for sessions: [SessionSummary]) -> [CatalogParseStatusFilterOption] {
    ...
    private static func matchesSearch(_ session: SessionSummary, searchText: String) -> Bool {
    ...
    private static func searchableMetadata(for session: SessionSummary) -> [String] {
        [
            session.title,
    ...
    private static func matchesProject(_ session: SessionSummary, filter: CatalogProjectFilter?) -> Bool {
    ...
    private static func matchesProfile(_ session: SessionSummary, filter: CatalogProfileFilter?) -> Bool {
        guard let filter else {
            return true
        }
    ...
    private static func matchesSource(_ session: SessionSummary, filter: CatalogSourceFilter?) -> Bool {
        guard let filter else {
            return true
        }
    ...
    private static func matchesParseStatus(
